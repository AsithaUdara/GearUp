...
if [[ ! -f "$LOCAL_COMPOSE_ENV" ]]; then
  ...
  cp "$ENV_FILE" "$LOCAL_COMPOSE_ENV"
  {
    ...
    echo "MODIFICATION_DB_URL=jdbc:postgresql://db:5432/as_modification_service"
    echo "MODIFICATION_DB_USER=svc_modification_service"
    echo "MODIFICATION_DB_PASSWORD=modification_svc_pass_2024"
