...
- Two services are available now:
  - Vehicle Service (port 8090)
  - Customer Service (port 8088)
...
- During local dev without the Gateway you can call services directly on 8090/8088; pass `X-User-ID` yourself for vehicle creation.
...
- `NEXT_PUBLIC_API_BASE_VEHICLE=http://localhost:8090` and `NEXT_PUBLIC_API_BASE_CUSTOMER=http://localhost:8088` (direct to services)
...
Base URL:
- Direct: `http://localhost:8088/api/customers`
...
# Get customer (Direct)
curl http://localhost:8088/api/customers/uid123
...
Base URL:
- Direct: `http://localhost:8090/api/vehicles`
...
- **Direct to Services (local dev fallback):**
  - Call `http://localhost:8090` (vehicles) and `http://localhost:8088` (customers).
...
- Vehicle health: `http://localhost:8090/actuator/health` → `{ "status": "UP" }`
- Customer health: `http://localhost:8088/actuator/health` → `{ "status": "UP" }`
