...
- Each folder under `services/` (e.g. `template-service`, `notification-service`, `parts-service`) is a Spring Boot microservice.
...
- Database initialization scripts are in `deployment/postgres/init-db.sql`
- Tables are created automatically on first container startup
